...
type ProductRepository interface {
	...
	FindAll(searchName string, page int, totalRows int) ([]model.Product, error)
...
func (p *productRepository) FindAll(searchName string, page int, totalRows int) ([]model.Product, error) {
	limit := totalRows
	offset := (page - 1) * limit
	var products []model.Product
	err := p.db.Select(&products, utils.ProductQueries.SelectAll, searchName, limit, offset)
	if err != nil {
		return nil, err
	}
	return products, nil
